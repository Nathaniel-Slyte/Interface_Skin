...
class MuCa {
  public:
    ...
    //RAW
    bool useRaw = false;
    int grid[ROWS_USE * NUM_COLUMNS];

    // manual callibration
    void calibrate();
    void setGain(int val);
    String message_ble = "Safe";
    void getRawData();
  ...
  private:
    ...
    //RAW
    //void getRawData();
    int calibrateGrid[ROWS_USE * NUM_COLUMNS];
...
void MuCa::getRawData() {
  ...
  // Read Data
  for (int rowAddr = 0; rowAddr < ROWS_USE; rowAddr++) {
    ...
    for ( int col = 0; col < NUM_COLUMNS; col++) {
      int output = (result[2 * col] << 8) | (result[2 * col + 1]); // Get High and Low bytes for an intersection of row and column
